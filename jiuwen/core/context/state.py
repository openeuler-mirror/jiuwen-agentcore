...
class State(ABC):
    def __init__(self, io_state: CommitState, global_state: CommitState, trace_state: StateLike, comp_state: StateLike):
    ...
    def update_io(self, data: dict) -> None:
        if self._io_state is None:
            return
        self._io_state.update(data)

    def update_trace(self, data: dict) -> None:
        if self._trace_state is None:
            return
        self._trace_state.update(data)
    ...
    def set_outputs(self, outputs: dict)-> None:
        ...
        return self._io_state.update(outputs)
