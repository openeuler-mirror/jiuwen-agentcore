...
class BranchComponent(WorkflowComponent, Executable):
    ...
    def invoke(self, inputs: Input, context: Context) -> Output:
        if self._executable is None:
            return self._executor.invoke(inputs, context)
        return inputs
    ...
    def add_component(self, graph: Graph, node_id: str, wait_for_all: bool = False, inputs: dict = None):
        graph.add_node(node_id, self.to_executable(), wait_for_all=wait_for_all, inputs=inputs)
